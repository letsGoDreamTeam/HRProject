...
def _calc_confidence(
    info: dict, final_edu: str, career_period: str, career_company: str, career_job: str
) -> str:
    """핵심 필드 충족도로 추출 신뢰도 산정"""
    score = 0
    core = [info.get("이름", ""), info.get("연락처", ""), info.get("이메일", ""), info.get("생년월일", "")]
    score += sum(1 for v in core if v)
    if final_edu and not any(k in final_edu for k in ["단과대학", "학교명", "학력사항"]):
        score += 2
    if career_period:
        score += 1
    if career_company:
...
def extract_from_docx(filepath: str, original_filename: str) -> dict:
    ...
    parsed = {
        **info,
        ...
        "경력회사": career_company,
        "경력직무": career_job,
        "추출신뢰도": _calc_confidence(info, final_edu, career_period, career_company, career_job),
...
def main() -> None:
    ...
    for file_path in target_files:
        ...
        try:
            # DOCX는 python-docx로 직접 파싱 (우분투 LibreOffice 변환 불필요)
            if ext == ".docx":
                data = extract_from_docx(file_path, original_name)
                all_parsed.append(data)
                continue

            # HWP는 pyhwp 직접 파싱 우선 시도 (실패 시 LibreOffice 변환 폴백)
            if ext == ".hwp":
                try:
                    data = extract_from_hwp_pyhwp(file_path, original_name)
                    all_parsed.append(data)
                    continue
                except Exception as e:
                    print(f"[WARN] {original_name} pyhwp 직접 파싱 실패, PDF 변환으로 폴백: {e}")

            # 나머지 포맷은 LibreOffice → PDF 변환 후 PyMuPDF 파싱
            ...
            if current_pdf and os.path.exists(current_pdf):
                with fitz.open(current_pdf) as pdf_doc:
                    all_parsed.append(extract_from_pdf(pdf_doc, original_name))
